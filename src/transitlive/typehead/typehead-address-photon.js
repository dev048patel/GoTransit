!(function (root, $, Bloodhound) {
  ...
  //  return feature.properties.osm_value;
      return feature.properties.osm_key;
  };
  ...
  var _formatResultSupplier = function (formatType) {
	...
	if (feature.properties.osm_key != 'place') {        //**Transit Info centre is a place for some reason?
//	    if (feature.properties.osm_key != 'highway') {    **Got rid of this in URL
		if (feature.properties.osm_key != 'waterway') {
		    //Now particular exclusions - not sure if this is the best way but whatever
		    if (feature.properties.name != 'Cornwall Center') {
			if (feature.properties.name != 'Sandra Schimrler Centre') {
			    if (feature.properties.name != 'University of Regina Map') {
//				if (feature.properties.name != 'University of Regina' & feature.properties.osm_value == 'park') {
...
//          formatted += feature.properties.name; }
      ...
      if (formatted != undefined && formatted.length >= 26) {
      ...
      if (feature.properties.housenumber && formatted =="") { // &&
...
//	      if (feature.properties.osm_key == 'building') {
//          formatted += ' ' + feature.properties.street;
//          }

//              if (feature.properties.osm_key == 'shop') {
//          formatted += ' ' + feature.properties.street;
//         }
...
//      }
if (formatted != undefined && formatted.length >= 38) {
            formatted = formatted.substring(0,36);
            formatted += '..';
...
if (formatted != undefined){
  ...
  root.PhotonAddressEngine = function (options) {
    ...
    var bloodhound = new Bloodhound({
      ...
      remote: {
        url: url + '/api/?osm_tag=!highway&osm_tag=!leisure:park',
        sufficient: limit,
